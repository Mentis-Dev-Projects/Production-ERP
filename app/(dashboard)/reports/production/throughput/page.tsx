...
import { ProductionThroughputReport } from "@/components/reports/production/production-throughput-report"
...
type ProductionThroughputPageProps = {
...
export default async function ProductionThroughputPage({ searchParams }: ProductionThroughputPageProps) {
  ...
  const renderFilterPanel = (idSuffix: string) => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Scope throughput by range, production line, status, client, and department.</CardDescription>
      </CardHeader>
      <CardContent>
        <form action="/reports/production/throughput" className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <label className="space-y-2">
            ...
            <input
              name="client"
              list={`production-throughput-clients-${idSuffix}`}
              defaultValue={item.filters.client === "all" ? "" : item.filters.client}
              placeholder="All clients"
              className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
            />
            <datalist id={`production-throughput-clients-${idSuffix}`}>
          ...
          <label className="space-y-2">
            ...
            <input
              name="department"
              list={`production-throughput-departments-${idSuffix}`}
              defaultValue={item.filters.department === "all" ? "" : item.filters.department}
              placeholder="All departments"
              className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
            />
            <datalist id={`production-throughput-departments-${idSuffix}`}>
          ...
          <div className="flex gap-2 md:col-span-2 xl:col-span-5">
            ...
            <Button asChild type="button" variant="outline">
              <a href="/reports/production/throughput">
  ...
  return (
    <>
      <MobilePageShell title="Production Throughput" subtitle="Production movement report for created, completed, and overdue orders.">
        <DataAccessNotice meta={meta} />
        {renderFilterPanel("mobile")}
        <ProductionThroughputReport item={item} />
      </MobilePageShell>

      <div className="hidden min-h-screen lg:block">
        <AppHeader title="Production Throughput" />
