...
type MentexPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>
}

export default async function ExpandedMetalPage({ searchParams }: MentexPageProps) {
  ...
  const { items: sourceItems, meta } = await getSalesOrders({
    q: params.q,
  ...
  const items = sourceItems.filter((item) => (params.step !== "all" ? item.currentStep.toLowerCase() === params.step.toLowerCase() : true))
  ...
  const mobileItems = items.map((item) => ({
    ...
    blockingStep: item.blockingStep,
    status: item.status,
  }))
  ...
  return (
    <>
      <MobileRectagrid
        title="Mentex"
        subtitle="A phone-first production stream board dedicated to Mentex."
        items={mobileItems}
        meta={meta}
      ...
      <div className="hidden min-h-screen lg:block">
        <AppHeader title="Mentex Production" />
        <main className="space-y-6 p-6">
          ...
          <Card>
            ...
            <CardContent>
              {stepCounts.length === 0 ? (
                <div className="py-12 text-center text-sm text-muted-foreground">No Mentex jobs available.</div>
              ) : (
          ...
          <RectagridFilters steps={steps} basePath="/expanded-metal" />

          <Card>
            <CardHeader>
              <CardTitle>Mentex Production Jobs</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
              ...
              {items.length === 0 && <div className="py-12 text-center text-muted-foreground">No Mentex jobs found.</div>}
            </CardContent>
          </Card>
        </main>
      </div>
